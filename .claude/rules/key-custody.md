...
- Must keep on-disk state to the bearer cache under `~/.signet/cache/` (mode 0600, atomic write, keyed by broker URL AND the enrolled key's fingerprint, 16 hex of SHA-256(SPKI DER), so a re-enrolled key never serves a bearer minted for the old one; `internal/attest/cache.go`) plus, per named `--identity`, one opaque hardware-wrapped key blob at `~/.signet/se-<identity>.key` (Secure Enclave, macOS) or `~/.signet/tpm-<identity>.key` (TPM, any identity but the default `consumer`) — 0600 under a 0700 dir, written by `writeKeyBlob` (`internal/signer/keyblob.go`), shared by both backends. The TPM's default identity and PIV write no key file; their key lives entirely in the hardware (a fixed persistent handle, or a PIV slot).
