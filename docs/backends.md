...
- **Non-exportable key, per substrate.** The signing key is generated in hardware and never leaves it: the Secure Enclave returns only an opaque hardware-wrapped blob; the TPM's default-identity key stays at its persistent handle, and a named identity's key is wrapped by the TPM and decrypted only transiently inside it on load; the PIV key stays in the selected slot on the token. In all three cases the private key never appears in a file, an env var, a log, or `argv`.
- **What persists.** signet holds no long-lived secret. The only persistent state is the short-lived bearer cache (all backends), the Secure Enclave's opaque, machine-bound key blob, and a named TPM identity's opaque, TPM-wrapped key blob. Either blob is useless if copied off the machine, because only the hardware that wrapped it can unwrap it. The TPM's default identity and the PIV backend write nothing to disk but the bearer cache. Deleting the cache simply forces a re-attestation.
