...
A binding's public key is read from hardware once and held in memory for the rest of the agent's life — it cannot change without a restart — so a burst of clients all asking for it at session start (every `headers`-helper row in a Claude Code project resolves the same identity) costs the card one open, not one per client, however many ask at once or how many times each one asks. A failed read is never cached, so a transient "card busy" answer stays retryable.

On the PIV backend, that mutex only ever prevents this agent's _own_ requests from overlapping — it cannot stop a genuinely separate process (this host's `ssh-agent` signing over the same physical YubiKey is the recorded case) from holding the PC/SC connection at the same instant. Opening the card while another connection holds it is refused by PC/SC itself ("the smart card cannot be accessed because of other connections outstanding"), so signet retries that specific, transient failure for up to ~2 seconds (a few hundred milliseconds per try) before giving up, and a client on `--agent` retries the whole request a few more times on a busy answer from the agent. A card genuinely held that long surfaces as a distinctly worded "card busy" failure, never as "no key enrolled" — the two are not the same thing, and only the second means re-enrol.
